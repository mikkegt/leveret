...
#### memo: model 層って何だっけ（毎回忘れる用）

`model/alert.go` に Attribute を足すときに「model ってロジック置くとこだっけ？」と毎回迷うのでメモ。このリポの `pkg/model/` は **データ定義だけの置き場**。Alert / Attribute / AttributeType みたいな struct と型を置くだけで、ビジネスロジックも DB アクセスも入れない（あっても Validate くらい）。

迷う原因は「model」という言葉が文脈で別物を指すから。

| 用語 | そこでの「Model」の意味 |
|---|---|
| MVC（Rails等） | データ + ビジネスロジック + DBアクセスまで全部入り（Fat Model）。「model＝ロジック」の記憶はこれ |
| DDD | ドメインモデル。エンティティ/値オブジェクトに振る舞いも持たせる |
| このリポ | データ定義だけ。ロジックは usecase、DB は repository に分離 |

このリポの model は MVC/DDD の Model よりやせている。データの器に徹していて振る舞いを持たない。

層の対応:
- model = データの形（名詞）
- repository = そのデータを保存/取得（DB＝Firestore 担当）← 「DBとやりとりするやつ」はこっち
- usecase = ビジネスロジック（手続き・動詞）
- adapter = 外部API（Gemini等）担当

C++アナロジー: `pkg/model/` はヘッダで struct だけ定義してる感じ（POD的）。メソッド満載のクラスじゃなく、データの構造体。振る舞いは usecase の関数が受け持つ。Goは「データと振る舞いを分けて書く」のが馴染むのでこういう薄い model になりやすい。

なお、このリポは厳密な DDD ではなく**レイヤードアーキテクチャ**（層分けだけ借りた形）。「ドメインモデルに振る舞いを寄せる」という DDD の核は採っていない。

覚え方: **model = 名詞だけ、動詞は usecase、保存は repository**。
